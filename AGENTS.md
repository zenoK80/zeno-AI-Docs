...
이 저장소는 Nextra 4와 Next.js로 만든 초보자용 학습 문서 사이트다. Codex는 `plan-codex/`의 과목을 담당하며, 현재 범위는 독학사 1단계 일반수학과 기초통계학이다.

## 역할과 변경 범위

- 작업 전 해당 `plan-codex/*.md`의 범위와 목차를 읽는다.
- 요청받은 과목의 MDX와 같은 폴더의 `_meta.js`만 수정한다.
- `plan-claude/`와 웹 기술 콘텐츠는 별도 요청이 없으면 건드리지 않는다.
...
- 공식은 기호의 뜻을 먼저 밝힌 뒤 작은 숫자로 끝까지 계산한다. 비교는 짧은 표, 흐름은 필요한 경우 Mermaid를 사용한다.
