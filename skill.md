...
JavaScript·React 등 웹 기술 학습 문서를 작성할 때 따르는 공통 규칙이다. 상세 MDX 형식·파싱 규칙은 `CLAUDE.md`를 함께 따른다. 대상 범위는 `plan-skill/`의 계획 파일들을 따른다.
...
- 일반 학습 파일은 본문 10~15분, 마무리 복습 5~8분 분량으로 쓴다(`CLAUDE.md` 기준과 동일).
- 한 편에는 직접 연결되는 핵심 개념 2~4개만 담는다. 긴 도입과 반복 요약은 뺀다.
- 처음 나오는 용어는 그 자리에서 쉬운 말로 설명한다. 따로 검색해야 문장을 이해하는 상태를 만들지 않는다. 약어로 된 핵심 개념 이름은 `CLAUDE.md`의 어원 분해 표 규칙을 따른다.
- 설명은 `왜 필요한가 → 쉽게 말하면 → 정의 → 작은 예시 → 계산/적용 → 결과 해석 → 자주 틀리는 점` 순서를 따른다.
...
- 흐름·상태·관계는 `Table` 또는 `Steps`로 표현한다(Mermaid는 이 프로젝트에서 작동하지 않음). 이벤트 루프·DOM·네트워크처럼 사용자가 노드를 움직이며 살펴볼 가치가 있을 때만 React Flow를 사용한다.
- 성능 수치나 시간에 따른 값은 Recharts로 표현한다. KaTeX는 알고리즘 수식처럼 수학 표기가 필요한 경우에만 사용하며 JSXGraph는 JavaScript 수치·그래픽 실습 외에는 강제하지 않는다.
...
- 전역 `<Quiz>`로 4지선다 문제를 만든다. 일반 학습 파일은 5~8개, 기출·모의 파일은 15~25개(`CLAUDE.md` 기준과 동일). 개념 확인, 코드 결과 판단, 흔한 오해를 섞는다.
...
- 모든 MDX는 예외 없이 맨 아래에 `## 참고 자료` 섹션을 작성한다. 실제 작성에 사용한 공식·신뢰 자료(MDN, 공식 스펙, 프레임워크 공식 문서 등)만 링크한다.
