...
`scripts/` 코드가 **어떻게 구성되어 있고 왜 그렇게 짰는지** 설명하는 문서입니다.
(스크립트는 이미 구현되어 있음 — **실행 방법은 [run-generate-doc.md](run-generate-doc.md)** 참고.
직접 다시 만들어 보며 익히고 싶다면 이 문서를 장 순서대로 따라가면 됩니다.)
