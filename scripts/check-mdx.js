...
//    (빌드 전에 미리 걸러내기 위함 — 특히 "볼드 안 괄호"는 빌드를 통과하고
//     렌더링만 깨지므로 사람 눈으로는 못 잡는다)
...
const RULES = [
  {
    // ** 로 줄을 쪼개면 홀수 번째 조각이 "볼드 안쪽"이다.
    // (**공개키**(public key) 처럼 볼드 밖 괄호는 정상이므로 오탐을 피하려면 이 방식이 필요)
    // 괄호가 "쌍"으로 들어 있을 때만 깨진다. 단독 ) 는 정상 렌더링되므로 잡지 않는다
    // (브라우저 실측으로 확인: **1) 계산이 싸다** 는 정상)
    name: '볼드 안 괄호 (렌더링 깨짐)',
    find: (line) => {
      const parts = line.split('**')
      if (parts.length < 3) return null
      for (let i = 1; i < parts.length - (parts.length % 2 === 0 ? 1 : 0); i += 2) {
        if (/\([^)]*\)/.test(parts[i])) return `**${parts[i].slice(0, 50)}**`
      }
      return null
    },
  },
