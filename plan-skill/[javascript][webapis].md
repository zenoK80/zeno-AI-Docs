...
- DOM·이벤트·Fetch·Storage처럼 코드를 수정하며 브라우저 결과를 확인해야 하는 편은 Sandpack 기반 `<CodePlayground template="vanilla">` 실습을 넣는다.
- MDX·JSX 내용을 클라이언트에서 렌더링하는 예제는 Nextra `Playground`를 사용한다. 일반 JavaScript 실행 실습은 CodePlayground를 사용해 두 도구의 역할을 구분한다.
- 지원 범위, 보안 조건, 사용자 권한이 필요한 API는 그 자리에서 짚는다.
- 비슷한 API는 선택 기준을 표로 비교하고, 요청·이벤트·렌더링 흐름은 Mermaid로 보여 준다.
- DOM 트리, 이벤트 전파, 네트워크처럼 직접 탐색하는 편은 React Flow를 선택하고 성능 측정값은 Recharts로 표현한다.
- Canvas 좌표나 애니메이션의 수학적 움직임은 필요한 경우 JSXGraph, 계산식은 MathJax를 사용한다.
