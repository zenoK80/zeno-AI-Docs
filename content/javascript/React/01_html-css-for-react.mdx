...
### 자주 나오는 한자어 뜻도 같이 알아두기

영단어뿐 아니라, 설명에 쓰이는 한자어 용어도 한자 뜻을 알면 훨씬 잘 와닿습니다.

| 용어 | 한자 | 뜻 풀이 |
|------|------|---------|
| 구조(構造) | 얽을 **구**(構) + 지을 **조**(造) | "얽어서 짓다" → 부분들이 서로 얽혀 이루는 짜임새 |
| 요소(要素) | 중요할 **요**(要) + 본디 **소**(素) | "중요한 본바탕" → 무언가를 이루는 기본 구성 성분 |
| 속성(屬性) | 붙을 **속**(屬) + 성질 **성**(性) | "붙어 있는 성질" → 어떤 대상에 딸려 있는 고유한 특징 |
| 문단(文段) | 글 **문**(文) + 층계 **단**(段) | "글의 층계(단락)" → 하나의 생각으로 묶인 글 덩어리 |
| 계층(階層) | 섬돌 **계**(階) + 층 **층**(層) | "계단의 층" → 위아래로 순서가 나뉜 단계 |
| 배치(配置) | 나눌 **배**(配) + 둘 **치**(置) | "나누어 두다" → 여러 요소를 각자의 자리에 벌여 놓음 |

> **쉽게 말하면:** "구조"는 "짜임새", "요소"는 "구성 성분", "속성"은 "딸려 있는 특징"으로 바꿔 읽으면 훨씬 쉽게 이해됩니다. 뒤에 나올 문서에서도 이 한자어들이 계속 등장하니 여기서 뜻을 잡아두면 좋습니다.
