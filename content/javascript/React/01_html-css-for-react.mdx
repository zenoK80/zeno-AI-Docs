...
HTML(HyperText Markup Language, 하이퍼텍스트 마크업 언어)은 웹 페이지의 **뼈대**를 담당합니다. 내용을 **태그(tag)**로 감싸서 "이 내용이 무슨 의미인지"를 브라우저에게 알려줍니다.
...
- `<!DOCTYPE html>`: "이 문서는 최신 HTML5 표준을 따른다"고 브라우저에 알리는 선언. 항상 첫 줄에 옵니다.
- `<html>`: 문서 전체를 감싸는 최상위 태그. `lang="ko"`는 "이 문서는 한국어"라는 뜻으로, 스크린 리더나 번역기가 참고합니다.
- `<head>`: 화면에 보이지 않는 메타 정보(제목, 문자 인코딩, 연결할 CSS 파일 등)가 들어갑니다.
- `<body>`: 실제로 화면에 보이는 모든 내용이 들어갑니다.
...
| 태그 | 용도 | 예시 |
| --- | --- | --- |
| `h1`~`h6` | 제목(숫자가 작을수록 더 큰 제목) | `<h1>가장 큰 제목</h1>` |
| `p` | 문단(paragraph) | `<p>텍스트</p>` |
| `div` | 의미 없는 블록 컨테이너(레이아웃용) | `<div>...</div>` |
| `span` | 의미 없는 인라인 컨테이너(텍스트 일부 강조용) | `<span>강조</span>` |
| `ul` / `ol` / `li` | 순서 없는 목록 / 순서 있는 목록 / 목록 항목 | `<ul><li>항목</li></ul>` |
| `button` | 클릭 가능한 버튼 | `<button>클릭</button>` |
| `input` | 사용자 입력창 | `<input type="text" />` |
| `img` | 이미지 삽입 | `<img src="..." alt="설명" />` |
| `a` | 다른 페이지로 이동하는 링크 | `<a href="...">텍스트</a>` |
...
CSS(Cascading Style Sheets, 종속형 스타일 시트)는 HTML 요소의 **색상, 크기, 위치**를 지정합니다.
