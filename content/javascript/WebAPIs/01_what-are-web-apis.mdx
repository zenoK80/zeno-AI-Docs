...
**API**(Application Programming Interface) 라는 이름을 단어별로 쪼개보면 무슨 뜻인지 훨씬 명확해집니다.
...
<Table>
  ...
  <tbody>
    <tr><td>Application</td><td>응용 프로그램(우리가 만드는 웹페이지, 앱)</td></tr>
    <tr><td>Programming</td><td>프로그래밍(코드를 짜는 행위)</td></tr>
    <tr><td>Interface</td><td>inter(사이)+face(경계면) → 두 시스템이 만나는 접점</td></tr>
  </tbody>
</Table>

API = "프로그램을 짤 때 다른 시스템과 만나는 접점". 즉, 다른 프로그램의 기능을 정해진 방법으로 사용하는 연결 창구입니다. 브라우저 내부를 직접 만들지 않아도 공개된 이름과 사용법을 따르면 기능을 쓸 수 있습니다.

> **쉽게 말하면:** API는 식당의 "메뉴판"과 같습니다. 주방(브라우저 내부)이 어떻게 요리하는지 몰라도, 메뉴판(API)에 적힌 이름으로 주문하면 원하는 결과를 받을 수 있습니다.
...
브라우저는 JavaScript 엔진으로 언어 코드(ECMAScript)를 실행하면서, 화면(DOM)·입력(Event)·통신(Fetch)·저장(Storage) 기능을 객체와 메서드 형태로 연결합니다.
