...
<Callout type="info">
  31편에서 만든 컨테이너에서 진행합니다. 아직 안 만들었다면 31편을 먼저 보세요. 이번 실습은 계정을 새로 만들어야 하므로, DBeaver 접속 정보의 Username을 관리자 계정인 `system`으로 두고 진행합니다.
</Callout>
...
<Quiz
  questionNumber={2}
  question="GRANT SELECT ON 사원 TO 김대리; (WITH GRANT OPTION 없음) 이후, 김대리가 GRANT SELECT ON 사원 TO 이과장;을 실행하면 어떻게 되는가?"
