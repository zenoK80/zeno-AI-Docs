...
<Callout type="info">
  31편에서 만든 Docker + Oracle 실습 환경에서 진행합니다. 아직 안 만들었다면 31편을 먼저 보세요. seed.sql이 만든 표는 건드리지 않고, 아래에서 새로 만드는 실습 전용 표에서만 진행합니다.
</Callout>
...
<Quiz
  questionNumber={3}
  question="빈 테이블에 다음을 순서대로 실행했다. 마지막 SELECT의 결과로 옳은 것은? INSERT VALUES(1); INSERT VALUES(2); SAVEPOINT sv1; UPDATE 번호=4 WHERE 번호=1; SAVEPOINT sv2; INSERT VALUES(5); DELETE WHERE 번호가 4 이상; ROLLBACK TO sv2; INSERT VALUES(6); COMMIT; SELECT * FROM 테이블;"
...
<Quiz
  questionNumber={6}
  question="INSERT INTO 주문 VALUES(1, 100, 5000); SAVEPOINT sv1; INSERT INTO 주문 VALUES(2, 101, 12000); CREATE TABLE 임시로그(기록 VARCHAR2(50)); ROLLBACK TO sv1; 을 실행하면 어떤 일이 일어나는가?"
